...
#define FIRMWARE_VERSION        "5.5.2-C3-OFF-FIX"
...
void handleConfigPortal(){
  ...
  // Same-origin REST/WS removes GitHub Pages HTTPS -> ESP32 HTTP mixed-content blocking.  if(!provisioningMode){
...
void apiMood(){
  ...
  if(server.hasArg("linkMode")) moodLinkMode=(MoodLinkMode)constrain(server.arg("linkMode").toInt(),0,3);  saveConfig(false);
  server.send(200,"application/json","{\"ok\":true}");
}
/* ===== LED TEST + OTA =================================================== */
...
stop:
  ...
  // FONTOS: a /1/ambilight/processed válasza régi/cached képadatot is adhat
  // akkor, amikor a TV már OFF. Ezért egy sikeres képkérés NEM írhatja felül
  // a /1/system/power által megállapított OFF állapotot. A power-state az
  // egyetlen autoritatív forrás a TV kimeneti engedélyezésére.
  // [FIX 5.5.2] tvOutputOff maradjon érintetlen.
...
void clearTVLedOutput(){
  // A TV OFF állapotnak azonnali, smoothing-független fizikai fekete kimenete van.
  // Nem várjuk meg, hogy currentZones lassan nullára simuljon, és a mood engine
  // sem kap lehetőséget arra, hogy az OFF állapot alatt visszakapcsolja a LED-eket.
  for(int i=0;i<4;i++){
    targetZones[i]=ZoneRGB{0,0,0};
    currentZones[i]=ZoneRGB{0,0,0};
  }
...
void pollTVMaster(){
  ...
  if(n>0){
    ...
    if(!deserializeJson(d,tvJsonBuf)){
      ...
      bool newOff = (strcmp(pw,"On")!=0);
      bool wasOff = tvOutputOff;
      tvOutputOff = newOff;
      if(newOff && !wasOff){
        Serial.println("[TV] power OFF -> LED output CLEAR");
        clearTVLedOutput();
      }
...
void handleTVWatchdog(){
  ...
  if(tvOnline){
    bool tooManyFails = (tvConsecutiveFailures>=TV_FAILURES_BEFORE_OFFLINE);    // stale csak akkor értelmezhető, ha már VOLT sikeres poll (lastSuccessfulPoll!=0)
...
void loop(){
  ...
  if(tvFetchAllowed && now-lastFrameStart>=TV_FRAME_INTERVAL_MS){
    ...
    smartFrameSeq++;
    // TV OFF állapotban a LED-kimenet mindig fekete.
    // Az Ambilight endpoint ilyenkor is adhat utolsó/cached frame-et, ezért
    // sem target, sem mapper, sem smoothing nem írhat vissza fényt.
    if(tvOutputOff){
      if(tvOnline) tvOnline=false;
      clearTVLedOutput();
    } else if(tvOnline){
      renderZonesToLeds();
      showLeds();