...
/* A fizikai címzés fix: 4 oldal × 30 LED, minden oldal 3 × 10 LED szegmens.
   A Mapperben csak a forrás, fényerő és irány módosítható. */
/* ── Smart Engine Panels ─────────────────────────────────────── */
...
function canonicalMapperDefaults(){
  const out=Array.from({length:FIXED_MAPPER_SEGMENTS},(_,i)=>({
    start:i*FIXED_MAPPER_LEDS,count:FIXED_MAPPER_LEDS,source:[1,1,1,2,2,2,3,3,3,4,4,4][i],
